...
export function buildPlayerCard(overrides?: Partial<PlayerCard>): PlayerCard {
  ...
  return {
    ...
    ratings: {
      offense: faker.number.int({ min: 50, max: 99 }),
      defense: faker.number.int({ min: 50, max: 99 }),
      physical: faker.number.int({ min: 50, max: 99 }),
      mentality: faker.number.int({ min: 50, max: 99 }),
    },
